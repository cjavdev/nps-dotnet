...
public class ArticleListParamsTest : TestBase
{
    [Fact]
    ...
    {
        ArticleListParams parameters = new()
        {
            Limit = 0,
            ParkCode = ["string"],
            Q = "q",
        ...
        Assert.Equal(
            new Uri(
                "https://developer.nps.gov/api/v1/articles?limit=0&parkCode=string&q=q&start=0&stateCode=string&api_key=My+API+Key"
            ),
            url
        );
    }
