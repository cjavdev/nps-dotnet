...
public class MultimediaListVideosParamsTest : TestBase
{
    [Fact]
    ...
    {
        MultimediaListVideosParams parameters = new()
        {
            Limit = 0,
            ParkCode = ["string"],
            Q = "q",
        ...
        Assert.Equal(
            new Uri(
                "https://developer.nps.gov/api/v1/multimedia/videos?limit=0&parkCode=string&q=q&start=0&stateCode=string&api_key=My+API+Key"
            ),
            url
        );
    }
