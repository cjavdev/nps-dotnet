...
public class GalleryListParamsTest : TestBase
{
    [Fact]
    public void FieldRoundtrip_Works()
    {
        var parameters = new GalleryListParams
        {
            Limit = 0,
            ParkCode = ["string"],
            Q = "q",
    ...
    {
        GalleryListParams parameters = new()
        {
            Limit = 0,
            ParkCode = ["string"],
            Q = "q",
        ...
        Assert.Equal(
            new Uri(
                "https://developer.nps.gov/api/v1/multimedia/galleries?limit=0&parkCode=string&q=q&start=0&stateCode=string&api_key=My+API+Key"
            ),
            url
        );
    }
    ...
    public void CopyConstructor_Works()
    {
        ...
        GalleryListParams copied = new(parameters);
