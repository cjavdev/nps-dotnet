...
public class VisitorCenterListParamsTest : TestBase
{
    [Fact]
    public void FieldRoundtrip_Works()
    {
        var parameters = new VisitorCenterListParams
        {
            Limit = 0,
            ParkCode = ["string"],
            Q = "q",
    ...
    {
        VisitorCenterListParams parameters = new()
        {
            Limit = 0,
            ParkCode = ["string"],
            Q = "q",
        ...
        Assert.Equal(
            new Uri(
                "https://developer.nps.gov/api/v1/visitorcenters?limit=0&parkCode=string&q=q&sort=string&start=0&stateCode=string&api_key=My+API+Key"
            ),
            url
        );
    }
    ...
    public void CopyConstructor_Works()
    {
        ...
        VisitorCenterListParams copied = new(parameters);
