...
public class LessonplanListParamsTest : TestBase
{
    [Fact]
    ...
    {
        LessonplanListParams parameters = new()
        {
            Limit = 0,
            ParkCode = ["string"],
            Q = "q",
        ...
        Assert.Equal(
            new Uri(
                "https://developer.nps.gov/api/v1/lessonplans?limit=0&parkCode=string&q=q&sort=string&start=0&stateCode=string&api_key=My+API+Key"
            ),
            url
        );
    }
